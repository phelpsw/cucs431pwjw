...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
        ...
        <xsl:element name="cs431:mashup" namespace="http://cs431.org/mashup#">
            ...
            <xsl:element name="cs431:mashupMetadata">
                <xsl:element name="dc:title">
                    <xsl:value-of select="document('queryholder.xml')/queryinfo/queryTarget"/>
                </xsl:element>
    ...
    <xsl:template match="/nsdl:NSDLSearchService" mode="nsdl">
        <xsl:for-each select="nsdl:SearchResults/nsdl:results/nsdl:document">
            <xsl:element name="cs431:querySource">
                <xsl:attribute name="query"><xsl:value-of select="document('queryholder.xml')/queryinfo/query[@src='nsdl']/@query"/></xsl:attribute>
    ...
    <xsl:template match="/rss" mode="yahoo">
        <xsl:for-each select="/rss/channel/item">
            <xsl:element name="cs431:querySource">
                <xsl:attribute name="query"><xsl:value-of select="document('queryholder.xml')/queryinfo/query[@src='yahoo']/@query"/></xsl:attribute>
                <xsl:element name="cs431:loc">
    ...
    <xsl:template match="/atom2:feed" mode="google">
        <xsl:for-each select="atom2:entry">
            <xsl:element name="cs431:querySource">
                <xsl:attribute name="query"><xsl:value-of select="document('queryholder.xml')/queryinfo/query[@src='google']/@query"/></xsl:attribute>
                <xsl:element name="cs431:loc">
